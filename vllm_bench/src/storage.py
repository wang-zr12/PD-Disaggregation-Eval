"""Persist metrics + traces. Always also writes ``*_latest.parquet`` for plots."""
...
def save_run(metrics: List[dict], traces: List[dict], results_dir: str,
             tag: str, timestamp: str = None) -> Tuple[pd.DataFrame, pd.DataFrame, dict]:
    """Write metrics (csv+parquet+json) and traces (parquet) under ``tag``.

    Also overwrites ``{tag}_metrics_latest.parquet`` / ``{tag}_traces_latest.parquet``
    so downstream plotting/analysis cells can find the most recent run.
    """
...
def load_latest(results_dir: str, tag: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
    m = pd.read_parquet(f"{results_dir}/{tag}_metrics_latest.parquet")
    t = pd.read_parquet(f"{results_dir}/{tag}_traces_latest.parquet")
    return m, t
